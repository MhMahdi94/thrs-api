...
const EmployeeEdit = () => {
    ...
    const handleEditEmployee = (e) => {
       ...
       // if (step == 0) {
        // console.log(formData);
            axiosClient.put(`/employees/${id}`, formData)
                .then(({ data }) => {
        console.log(data);
                    setLoading(false);
                   // setStep(1);
    ...
    const getEmployee=()=>{
        axiosClient.get(`/employees/${id}`)
            .then(({data})=>{
                console.log(data.user[0].mobileNo);
                setFormData(data.user[0]);
                setinfoData(data.infoData[0]);
                data.allowence.map(all=>                
                    setAllowances([...allowances,{allName:all.allName,allVal:all.allVal}])
                )
            })
            .catch(
                (error)=>{
